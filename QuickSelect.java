...
public class QuickSelect {
	/**
	 * Quick selection algorithm. Places the item closest to the k-th quantile
	 * in a[k*size()-1] and returns it.
	 * 
	 * @param a
	 *            an ArrayList of Double items.
	 * @param k
	 *            the desired rank from 0 (smallest) to 1 (largest).
	...
	/**
	 * Quick selection algorithm. Places the kth smallest item in a[k-1] and
	 * returns it.
	 * 
	 * @param a
	 *            an ArrayList of Double items.
	 * @param k
	 *            the desired rank (1 is minimum) in the entire ArrayList.
	 */
	 ...
	 */
	private static <T> void quickSelect(ArrayList<Double> a, int low, int high,
			int k) {
		if (low + CUTOFF > high)
	 ...
	 */
	public static final void swapReferences(ArrayList<Double> a, int index1,
			int index2) {
	...
	// Should return the index of the element of interest in quickselect. Seems
	// to be error prone.
	public int position(ArrayList<Double> arr, int value) {
		double valueToIndex = quickSelect(arr, value);
		for (int h = 0; h < arr.size(); h++) {
			if (arr.get(h) == valueToIndex) {
